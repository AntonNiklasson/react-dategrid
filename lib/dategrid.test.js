...
describe("Dategrid", () => {
  ...
  describe("Navigation", () => {
    ...
    it("should render the navigation using the render props if given", () => {
      const component = mount(
        <Dategrid
          renderPreviousNavigation={({ gotoPreviousMonth }) => (
            <marquee onClick={gotoPreviousMonth}>{"<<"}</marquee>
          )}
          renderNextNavigation={({ gotoNextMonth }) => (
            <marquee onClick={gotoNextMonth}>{">>"}</marquee>
          )}
        />
      );
